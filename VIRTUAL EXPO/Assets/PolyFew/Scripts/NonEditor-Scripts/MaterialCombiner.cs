...
namespace BrainFailProductions.PolyFew
{
    ...
    public class MaterialCombiner : MonoBehaviour
    {
        ...
        private static Dictionary<GameObject, List<MaterialEntity>> InitializeFromGameObjects(List<GameObject> gameObjectsList, TextureArrayGroup textureArraysSettings, DiffuseColorSpace diffuseColorSpace, bool regardChildren, Action<string> OnError)
        {
            ...
            {
                var textureArray = staticCombinationInfo.materialEntities[a];

                if (!textureArray.HasAnyTextures())
                {
...
#pragma warning disable
                ...
                Texture2DArray diffuseArray = AllocateArray(objectsData, settings.diffuseArraySettings, textureArraysCount, objectsData.diffuseColorSpace, diffuseHasAlpha);
                Texture2DArray normalArray = createNormalArr ? AllocateArray(objectsData, settings.normalArraySettings, textureArraysCount, DiffuseColorSpace.LINEAR, true) : null;
                Texture2DArray metallicArray = createMetalArr ? AllocateArray(objectsData, settings.metallicArraySettings, textureArraysCount, DiffuseColorSpace.LINEAR, true) : null;
                Texture2DArray occlusionArray = createOcclusionArr ? AllocateArray(objectsData, settings.occlusionArraySettings, textureArraysCount, DiffuseColorSpace.LINEAR, false) : null;
                Texture2DArray heightArray = createHeightArr ? AllocateArray(objectsData, settings.heightArraySettings, textureArraysCount, DiffuseColorSpace.LINEAR, false) : null;
                Texture2DArray emisArray = createEmissionArr ? AllocateArray(objectsData, settings.emissiveArraySettings, textureArraysCount, objectsData.diffuseColorSpace, false) : null;
                Texture2DArray detailAlbedoArray = createDetailAlbedoArr ? AllocateArray(objectsData, settings.detailAlbedoArraySettings, textureArraysCount, objectsData.diffuseColorSpace, false) : null;
                Texture2DArray detailNormalArray = createDetailNormalArr ? AllocateArray(objectsData, settings.detailNormalArraySettings, textureArraysCount, DiffuseColorSpace.LINEAR, true) : null;
                Texture2DArray detailMaskArray = createDetailMaskArr ? AllocateArray(objectsData, settings.detailMaskArraySettings, textureArraysCount, DiffuseColorSpace.LINEAR, false) : null;
                Texture2DArray specularArray = createSpecularArr ? AllocateArray(objectsData, settings.specularArraySettings, textureArraysCount, DiffuseColorSpace.LINEAR, true) : null;
                ...
                for (int a = 0; a < srcMatEntitites.Count; ++a)
                {
                    try
                    {
                        ...
                        CompressTexture(diffuseTexture, objectsData, settings.diffuseArraySettings, diffuseHasAlpha);
                        CompressTexture(normalTexture, objectsData, settings.normalArraySettings, true);
                        CompressTexture(metallicTexture, objectsData, settings.metallicArraySettings, true);
                        CompressTexture(occlusionTexture, objectsData, settings.occlusionArraySettings, false);
                        CompressTexture(heightTexture, objectsData, settings.heightArraySettings, false);
                        CompressTexture(emissionTexture, objectsData, settings.emissiveArraySettings, false);
                        CompressTexture(detailAlbedoTexture, objectsData, settings.detailAlbedoArraySettings, false);
                        CompressTexture(detailNormalTexture, objectsData, settings.detailNormalArraySettings, true);
                        CompressTexture(detailMaskTexture, objectsData, settings.detailMaskArraySettings, false);
                        CompressTexture(specularTexture, objectsData, settings.specularArraySettings, true);
...
#region HELPERS
        ...
        private static Texture2DArray AllocateArray(CombiningInformation combineInfo, TextureArrayUserSettings arraySettings, int texturesCount, DiffuseColorSpace diffuseColorSpace, bool hasAlphaChannel)
        {
            int width = arraySettings.resolution.width;
            int height = arraySettings.resolution.height;
            var textureFormat = GetTextureFormat(combineInfo, arraySettings.compressionType, arraySettings.compressionQuality, hasAlphaChannel);
        ...
        private static TextureFormat GetTextureFormat(CombiningInformation cfg, CombiningInformation.CompressionType cmp, CombiningInformation.CompressionQuality quality, bool hasAlphaChannel)
        {
            if (hasAlphaChannel)
            {
        ...
        private static Texture2D GetResizedTexture(Texture2D toResize, CombiningInformation.Resolution newSize, bool hasNonColorData)
        {
        ...
        private static void CompressTexture(Texture2D toCompress, CombiningInformation combineInfo, TextureArrayUserSettings arraySettings, bool hasAlphaChannel)
        {
            if (toCompress == null) { return; }

            TextureFormat textureFormat = GetTextureFormat(combineInfo, arraySettings.compressionType, arraySettings.compressionQuality, hasAlphaChannel);
            EditorUtility.CompressTexture(toCompress, textureFormat, 50);
            toCompress.Apply();
        }


        private static void WriteTextureToTextureArray(Texture2D toWrite, Texture2DArray dstTextureArray, int index, int mipCount)
        {
        ...
        private static int SizeToMipCount(TextureArrayUserSettings arrayProperties)
        {
